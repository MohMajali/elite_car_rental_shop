...
if (isset($_POST['Submit'])) {
    ...
    $query = mysqli_query($con, "SELECT * FROM offices WHERE name ='$name' AND email = '$email'");
    ...
    if (mysqli_num_rows($query) > 0) {
      ...
      alert ('Office Already Exist !');
        ...
        $stmt = $con->prepare("INSERT INTO offices (name, email, phone, image, address, description, password) VALUES (?, ?, ?, ?, ?, ?, ?) ");

        $stmt->bind_param("sssssss", $name, $email, $phone, $image, $address, $description, $password);

        if ($stmt->execute()) {

            $stmt = $con->prepare("SELECT id FROM offices WHERE name = ?");
            ...
            if ($stmt->num_rows > 0) {
                ...
                $stmt = $con->prepare("INSERT INTO office_subscriptions (office_id, subscription_type, start_date, end_date, price) VALUES (?, ?, ?, ?, ?)");

                $stmt->bind_param("issss", $id, $subscription_type, $start_date, $end_date, $price);

                if ($stmt->execute()) {

                    move_uploaded_file($_FILES["file"]["tmp_name"], "./Office_Dashboard/Offices_Images/" . $_FILES["file"]["name"]);
...
<html lang="en">
  <head>
    ...
    <title>Office Register Page</title>
              ...
              >
                ...
                <div class="card mb-3">
                  <div class="card-body">
                    ...
                    <form class="row g-3 needs-validation" method="POST" action="./Office_Register.php" enctype="multipart/form-data" id="login-form">
